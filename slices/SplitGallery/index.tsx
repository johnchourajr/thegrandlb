...
const SplitGallery = ({ slice }: SliceComponentProps): JSX.Element => {
  const ref = useRef<HTMLDivElement>(null);
  const { scrollYProgress } = useScroll({
    target: ref,
    offset: ["start end", "end start"],
  });
  const { section_id, top_spacer, bottom_spacer, gallery_left, gallery_right } =
    slice as any;
  const leftItems: ImageItem[] =
    (gallery_left as { data?: { gallery_items?: ImageItem[] } } | null)?.data
      ?.gallery_items ?? [];
  const rightItems: ImageItem[] =
    (gallery_right as { data?: { gallery_items?: ImageItem[] } } | null)?.data
      ?.gallery_items ?? [];

...
| `NEXT_PUBLIC_RESEND_FROM_EMAIL`   | No (has fallback)        | From address for emails. Workflow uses `noreply@example.com` if unset.                                           | `noreply@example.com`                                              |
| `NEXT_PUBLIC_RESEND_SALES_EMAIL`  | No (has fallback)        | Sales team recipient. Workflow uses `notifications@example.com` if unset.                                        | `notifications@example.com`                                        |
| `NEXT_PUBLIC_RESEND_REPLY_EMAILS` | No (has fallback)        | Reply-to address(es), comma-separated. Workflow uses `reply@example.com` if unset.                              | `reply@example.com`                                                |
| `CI_ERROR_NOTIFICATION_EMAIL`     | Recommended              | Email that receives an alert when the inquiry form CI test fails.                                                | `alerts@example.com`                                               |

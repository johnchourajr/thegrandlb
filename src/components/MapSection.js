import React from "react";

import PageSection from "./PageSection";
import Star from "./svg/Star";
import Map from "./Map";
import ScrollAnimate from "./ScrollAnimate";
import ReactGA from "react-ga";

function onClick(event) {
  if (event) {
    ReactGA.event({
      category: event.category,
      action: event.action,
    });
  }
}

// Component
const MapSection = (props) => {
  const map = {
    center: {
      lat: 33.8041368,
      lng: -118.1473096,
    },
    zoom: 11,
  };

  return (
    <div>
      <PageSection
        heading={props.heading}
        headingTag="h2"
        caption={props.caption}
        captionClassName="caption--small"
      />
      <div className="map--wrap">
        <Map map={map} />
      </div>
      <div className="map--footer xs-flex xs-flex-align-center xs-flex-column">
        <p className="h6 xs-text-center xs-mb3">Find Us</p>
        <a target="_blank" href={props.mapLink} rel="noopener noreferrer">
          <p className="h2 xs-text-center">
            {props.address1}
            <br />
            {props.address2}
          </p>
        </a>
      </div>
      <div className={"md-px6 md-mx6 xs-pt4 md-pt6 xs-pb3 clearfix"}>
        {props.features.map((item, i) => (
          <ScrollAnimate
            delay={150 * i}
            key={i}
            className={
              "col xs-pt3 xs-col-12 sm-col-4 xs-flex xs-flex-column xs-flex-align-center"
            }
          >
            <a
              href={item.url}
              onClick={() => {
                onClick(item.event);
              }}
              className="xs-flex xs-flex-column xs-flex-align-center"
              target="_blank"
              rel="noopener noreferrer"
            >
              <Star
                color={"rgba(0,0,0,.3)"}
                strokeWidth={2}
                width={32}
                height={32}
              />
              <h3 className="xs-text-center">{item.text}</h3>
            </a>
          </ScrollAnimate>
        ))}
      </div>
    </div>
  );
};

export default MapSection;

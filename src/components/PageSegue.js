    ...
    <ScrollAnimate className="section gutters clearfix no-print">
      {seguePages.map((item, i) => {
        if (currentPage !== item.key && i < 3) {
          return (
            <div key={i} className="col xs-col-12 md-col-6 segue-card">
              <div className="segue-card--img-wrap">
                <div
                  className="segue-card--img"
              ...
              <Link to={item.path}>
                <div className="segue-card--text">
                  <h3 className="h3 display">{item.headline}</h3>
                </div>
                <div className="segue-card--button-wrap">
                  <div className={`button button--secondary`} to={item.path}>
                    {item.buttonText}
                  </div>
                </div>
              </Link>
